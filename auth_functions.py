...
cred = credentials.Certificate('acumen-chat-academy-firebase-adminsdk-bbhkf-91a1287596.json')
...
def sign_in(email:str, password:str) -> None:
    try:
        ...
        # Save user info to session state and rerun
            ...
            st.session_state.user_type = data.to_dict()["type"]
            if st.session_state.user_type == "Student":
                # Redirect to student_main.py
                st.success("Logged in!")
                st.session_state.is_authenticated = True
                st.session_state.user_type = "Student"
                st.switch_page("pages/student_main.py")
                ...
                st.switch_page("pages/teacher_main.py")
            else:
        ...
        if error_message in {"INVALID_EMAIL","EMAIL_NOT_FOUND","INVALID_PASSWORD","MISSING_PASSWORD"}:
            ...
            st.session_state.auth_warning = 'Error: user not exist'

    except Exception as error:
        print(error)
        st.session_state.auth_warning = 'Error: Please Check your email and password'


def create_account(email:str, password:str, user_type:str) -> None:
    try:
